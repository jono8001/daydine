# Simla Takeaway — Monthly Intelligence Report
*2026-04 | Generated 09 April 2026 | DayDine Premium*

---

## Executive Summary

**What you should fix now:**

1. **Lean into food quality — it's what guests are buying** [EXPLOIT] — Sharpen proposition → attract the right guests
2. **Accelerate review volume — you're credible but not dominant** [EXPLOIT] — Improved Google Maps ranking
3. **Address isolated food quality complaints from guests** [FIX] — Reduce food quality complaints, protect rating

**Watch this month:**
- Monitor local competitor openings and closures
- Track Google rating trend month-over-month

**Do not prioritise:** Don't chase prestige before fixing fundamentals

**Overall score: 6.6/10** (baseline month — no prior comparison)
 | Strongest: Experience (8.8) | Weakest: Visibility (5.1)

**Competitive position:** Local Peers (5mi): #1 of 7 (P100.0) — peer avg 5.86, top 6.42
  Catchment (15mi): #1 of 16 (P100.0) — peer avg 5.56, top 6.57
  UK Category Cohort: #1 of 17 (P100.0) — peer avg 5.55, top 6.57

*This report uses publicly observable data only — no internal systems required. See Data Basis below for full evidence breakdown.*

## Next 30 Days — Demand Forecast

| Period | Signal | Driver | Recommended Action |
|---|---|---|---|
| 2–4 Apr | 🔴 PEAK | Bank holiday: Good Friday | Ensure full opening hours are correct on Google for Good Friday |
| 4–19 Apr | 🟡 HIGH | School: Easter holidays | Family-friendly offer visible — check kids menu and highchair mention on Google |
| 6–7 Apr | 🔴 PEAK | Bank holiday: Easter Monday | Ensure full opening hours are correct on Google for Easter Monday |
| 15–21 Apr | 🔴 PEAK | RSC: Hamlet opens | Confirm pre-theatre menu is live on Google before 15 Apr |
| 20–30 Apr | 🟢 STEADY | School: Summer term (term time) | — |
| 22–30 Apr | 🟡 HIGH | RSC: Hamlet in performance | Pre-theatre covers likely elevated — staff accordingly for 5:30–7pm service |

**Biggest opportunity this month:** Bank holiday: Good Friday (2–4 Apr) is your biggest demand opportunity this month. Your Tourists & Visitors, Locals segment converts on atmosphere and food quality — ensure your Google photos and menu are updated before the period starts.

### Financial Impact

Based on publicly observable data for Simla Takeaway, your current operational gaps are putting an estimated **3–30 covers per week** at risk — representing approximately **£270–£4,500 per month** in lost or leaked revenue. The main drivers are: Lean into food quality — it's what guests are buying (upside opportunity); Accelerate review volume — you're credible but not dominant (discovery protection); Address isolated food quality complaints from guests (£270–£4,500/mo). Taking action on **Lean into food quality — it's what guests are buying** could begin recovering this within weeks at minimal effort.

| Metric | Current | At Risk | Potential Recovery |
|---|---|---|---|
| Covers per week | ~262 | 3–30 at risk | Recoverable with profile fixes |
| Average spend per head | ~£22 | — | — |
| Weekly revenue impact | — | £70–£1,120 | £70–£1,120 recoverable |
| Monthly revenue impact | — | £270–£4,500 | £270–£4,500 recoverable |
| Annual projection | — | £3,240–£54,000 | £3,240–£54,000 recoverable |

*Estimates based on UK hospitality benchmarks for price level ££ venues. Ranges are directional — exact figures require your internal cover and spend data.*

**Recommended Action:** Lean into food quality — it's what guests are buying. Estimated cost: Low (< £200). Expected payback: within 1 month.

## Operational & Risk Alerts

> This section flags issues that go beyond normal review themes. These are patterns that could represent legal liability, operational failure, or reputational damage requiring direct action.

### No Operational or Risk Alerts This Period

No review patterns in the 5 reviews scanned indicate legal, safety, staffing, or operational risks above normal levels. This is a positive signal. Standard theme analysis follows below.

## Data Basis

This report draws on three layers of evidence:

| Layer | What | Count | Confidence |
|---|---|---|---|
| Deep analysis | Text reviewed, themes extracted, segments identified | 5 reviews (5 Google, 0 TripAdvisor) | High for recent themes, moderate for long-term patterns |
| Aggregate signals | Rating and volume from all platforms | 73 Google (4★), 0 TripAdvisor (None★) | High for reputation baseline, cannot identify specific themes |
| Structural data | FSA record, Google Business Profile, web presence | 3 sources | High — factual data, not subjective |

**What this means:** The narrative insights (themes, complaints, praise, segment reads) are based on 5 deeply-analysed recent reviews — a detailed sample, not a census. The aggregate 4 rating across 73 Google reviews is the reputation bedrock and is factored into scoring separately.

⚠️ **Data Confidence: Indicative.** This report is based on 5 reviews from 1 source(s), which is below our recommended minimum of 50. Findings should be treated as indicative rather than conclusive.

ℹ️ Reviews are from a single source (Google). Cross-platform validation is not possible.


> **Data Confidence Note:** This report is based on 5 reviews from 0 source(s), which is below our recommended minimum of 50. Findings should be treated as indicative rather than conclusive. Cross-source validation is not possible with a single data source.

## Monthly Movement Summary

### Seasonal Context

- **Stratford-upon-Avon — shoulder season.** Easter tourism + RSC spring programme.
- RSC theatre season is active — pre-theatre dining demand is elevated.

*This is the first report with month-over-month tracking. All metrics are baselined. Next month's report will show deltas, trends, and movement context.*

## What This Venue Is Becoming Known For

Simla Takeaway is a Indian dining venue with an adequate public reputation (4.0/5, 73 reviews) that has room to build stronger proof.

**From deep analysis of 5 recent reviews:**

Based on a limited sample of 5 reviews (Google's 'most relevant' selection), **food quality** (3 mentions) and **service & hospitality** (2 mentions) are the most frequent themes. Whether these represent settled guest perception or sampling artefact cannot be determined from this evidence base alone.

Within the local Indian dining market (6 peers), this venue leads the field. The identity is that of a category leader — the venue others are compared against.

The public proof (ratings, compliance) aligns with what guests actually experience. This consistency is commercially valuable — it means expectations set online are met in person.

## Who's Telling You What — Guest Segment Intelligence

*Different guests experience your venue differently. Here's what each group is telling you.*

**Data basis:** 1 of 5 reviews (20%) contained enough context to identify the guest type. Segments below require ≥2 reviews to surface a pattern.

### Watch List (single-review signals — not yet patterns)

- 1 review suggests **Locals & Regulars** may be present — *"We always have great food from Simla"*

### Unattributed Reviews (4)

Top themes across unattributed reviews: Food Quality (2x), Service & Hospitality (1x).

*As review volume grows, more segments will become readable.*

## Management Priorities

The score is solid but not dominant. The management question is *which specific lever, if pulled, would most change your commercial trajectory* — and which can you realistically move?

### Priority 1: Lean into food quality — it's what guests are buying [EXPLOIT | NEW]

Guests consistently praise food quality (3 positive mentions). This appears to be what the venue actually sells, whether or not it's what you think you sell. Make it the headline of your online presence.

**Management implication:** Your guests are telling you your proposition. Use it.

- **Owner:** management | **Confidence:** 70%
- **Expected upside:** Sharpen proposition → attract the right guests
- **Evidence:** `review_praise: Food Quality=3`

  **Commercial consequence** (indicative):
  Value at stake: Upside, not loss prevention — magnitude depends on execution | Cost to fix: Low (< 1 hour, no spend) | Payback: 1–3 months
  *Basis: Aligning online messaging to guest-validated strengths sharpens conversion*
- **Segment impact:** Couples and occasion diners have the highest expectations and write the most detailed negative reviews. Fixing food complaints protects your rating with the segment that amplifies problems fastest.

### Priority 2: Accelerate review volume — you're credible but not dominant [EXPLOIT | NEW]

At 73 reviews, your rating is stable but not commanding. Venues with 200+ reviews rank higher in Maps. A post-visit review prompt would compound monthly.

**Management implication:** Visibility drives discovery. A venue that isn't found can't convert. Typically moveable within 60 days.

- **Owner:** front-of-house | **Confidence:** 75%
- **Expected upside:** Improved Google Maps ranking
- **Evidence:** `grc=73`

  **Commercial consequence** (indicative):
  Value at stake: Commercial impact not robustly estimable from current external evidence. | Cost to fix: Low (< 1 hour, no spend) | Payback: 1–3 months
  *Basis: Visibility improvements compound over weeks, not days.*
- **Segment impact:** Tourists and first-time visitors depend entirely on your online presence. Protecting visibility protects your discovery pipeline.

### Priority 3: Address isolated food quality complaints from guests [FIX | NEW]

Food Quality draws isolated criticism across reviews (1 negative mentions). Investigate whether this reflects a systemic issue or isolated incidents.

**Management implication:** Guests are telling you what's wrong. Listen.

- **Owner:** operations | **Confidence:** 70%
- **Expected upside:** Reduce food quality complaints, protect rating
- **Evidence:** `review_criticism: Food Quality=1 negative`

  **Commercial consequence** (directional):
  Value at stake: £270–£4,500/month | Cost to fix: Medium (1–2 days or < £200) | Payback: 1–3 months
  *Basis: Recurring complaints reduce repeat visits and suppress rating trajectory*
- **Segment impact:** Couples and occasion diners have the highest expectations and write the most detailed negative reviews. Fixing food complaints protects your rating with the segment that amplifies problems fastest.

## Protect / Improve / Ignore

### Protect

- **Your 4.0/5 Google rating.** This is your primary commercial asset. Protect it through consistent delivery.

### Improve

- **Review momentum.** At 73 reviews, building toward the next milestone (100, 250, 500) directly improves Google Maps ranking. A systematic post-visit review prompt is the highest-ROI marketing action.

### Ignore

- **Awards and editorial pursuit.** Your fundamentals need strengthening before prestige investment makes sense. Awards don't fix a weak proposition; they amplify a strong one.

## Demand Capture Audit

**3 of 7** demand capture dimensions are clear. **4** have friction. **0** are missing.

*First audit — all dimensions baselined for next month's comparison.*

*Conversion dimension score: 6.0/10 — this audit explains what is behind that number.*

### Booking Friction [PARTIAL]

Booking is likely available via website, but no direct booking link is confirmed in Google Maps. A customer must leave Maps → find the website → locate the booking page. Each click loses ~20% of intent.

**Commercial consequence:** Customers searching 'restaurant near me' expect 1-click booking. Without it, you lose to competitors with Reserve with Google or a visible booking button.

*Signals: GBP type includes 'restaurant' (reservation expected); Website present (may host booking)*

### Menu Visibility [PARTIAL]

Menu exists online (inferred) but we cannot confirm it is linked from Google Maps or that it is scannable on mobile. 77% of diners check the menu before visiting — if it is buried on a sub-page or a PDF, it may as well be missing.

**Commercial consequence:** A menu that exists but isn't in the discovery path loses most of its conversion value.

*Signals: Menu online: detected (inferred from web presence); Website present (menu may be hosted there); 1 review(s) mention the menu*
> *"Takeaway only, no dine in option.

Food is excellent, fresh produce (fresh salad/sides, etc), well spiced and served pip..."*

### CTA Clarity [PARTIAL]

GBP profile is 8/10 complete. Missing: Has price level, Review count >= 100. Each gap removes a potential action path for a customer ready to commit.

**Commercial consequence:** Partial CTA completeness means some customers can't take their preferred action.

*Signals: GBP completeness: 8/10; Missing: Has price level, Review count >= 100*

### Photo Mix & Quality [PARTIAL]

Only 7 photos vs peer average of ~10. Low photo count suppresses click-through.

**Commercial consequence:** Venues with 10+ photos get ~35% more click-throughs (Google data).

*Signals: Photo count: 7; Peer average: ~10 photos; Top review theme: Food Quality*

### Proposition Clarity [CLEAR]

Google types (meal_takeaway, indian_restaurant, restaurant) are broadly consistent with what guests praise (Food Quality, Service & Hospitality).

**Commercial consequence:** No major proposition mismatch detected.

*Signals: Google types: meal_takeaway, indian_restaurant, restaurant; Top review themes: Food Quality, Service & Hospitality, Value For Money*

### Mobile Usability [CLEAR]

Opening hours complete (7/7 days), website present, Place ID active. A mobile user can confirm hours and navigate without leaving Maps.

**Commercial consequence:** No mobile usability friction detected.

*Signals: Opening hours: 7/7 days; Website: Yes; Google Place ID: Yes*

### Promise vs Path [CLEAR]

No contradictions detected between listing promises and available evidence.

**Commercial consequence:** Promise-path consistency appears intact.

*Signals: Delivery/takeaway flagged in Google types*

## Commercial Diagnosis

### Primary Constraint

**No single binding constraint.** The venue is operationally balanced. The commercial focus should be on marginal gains and protecting the strongest dimensions rather than crisis management.

### Secondary Drag

Online visibility (5.1) hasn't kept pace with guest experience quality (8.8). Good venues that aren't easily found online are subsidising competitors with better digital presence. This is fixable within 60 days.

### Hidden Risk

Google rating sits at 4.0/5 — the threshold where Google's algorithm shifts search priority. A few bad reviews could push you below 4.0, which would materially reduce discovery. Conversely, a small improvement would lift you into a stronger ranking band.

### What May Be Misleading

No significant false-comfort signals detected. The data picture appears consistent.

### Positioning

Category leader in the local market, with food quality as the emerging differentiator. This position supports premium pricing and extension opportunities. The risk is complacency — leadership must be actively maintained.

### Revenue Left on the Table

- **Unrealised review authority.** At 73 reviews, you lack the volume to dominate local search. 500+ reviews at similar ratings rank measurably higher in Google Maps.
- **Potential proposition signal.** Early review data (from a limited sample) suggests food quality resonates. If confirmed, this could become a marketable differentiator.

*2 revenue leaks identified. Most are fixable at low cost (profile updates, messaging changes). Combined value at stake is likely directional — exact figures require internal cover/spend data.*

## Review & Reputation Intelligence

*Reputation baseline: 5 reviews analysed (5 Google (undated)). This is the full available sample, not limited to the current month.*

### What Guests Value Most

Food Quality draws strong positive signal across reviews. Guests highlight specific dishes and ingredients rather than generic praise, which indicates the kitchen is delivering memorable experiences, not just adequate meals. As one reviewer put it: *"Food is excellent, fresh produce (fresh salad/sides, etc), well spiced and served piping hot"*

Service & Hospitality is a dominant theme (positive signal, limited evidence). Guests describe attentive, personal care — naming individual staff in several cases. This suggests the venue's competitive advantage is human rather than purely operational: it depends on who is working, not just what systems are in place. As one reviewer put it: *"Curries nice and thick sauce and the delivery guys are super friendly and always bring up to my flat for me"*

Value for Money shows positive signal, limited evidence. Guests feel the price-to-quality ratio works in their favour, which strengthens repeat visit likelihood and word-of-mouth. As one reviewer put it: *"Great value for money too...."*

### What Management May Be Missing

**Food Quality** (isolated mention, 1 mention).
This warrants investigation to determine whether it reflects a systemic issue or isolated incidents.

### Reputation Risk Assessment

**Trajectory warning:** Later reviews trend lower than earlier ones (based on sample order — not date-confirmed). A declining trajectory, if sustained, will erode the aggregate rating over time. This is an early warning — the operational cause should be identified before it becomes visible in headline ratings.

### Evidence Quality

**Confidence: Anecdotal** (5 from Google).
 This analysis draws on a very small sample. Themes are observed impressions, not confirmed patterns. Management should treat these findings as hypotheses to investigate, not conclusions to act on.
 Note: Google's API surfaces only its 'most relevant' reviews (max 5 per venue) from 73 total — this sample likely skews toward popular positive reviews.

### Recent Movement

*No dated reviews available — recent movement cannot be isolated.*

The analysis above reflects the full reputation baseline. A dated recent-movement layer will become available when reviews with reliable dates fall within the report period.

## Menu & Dish Intelligence

### What Guests Are Ordering and Mentioning

No specific dish mentions detected in current review sample. This may indicate reviews focus on service and atmosphere rather than individual dishes, or that the review volume is too low for dish-level extraction.

### What's Trending in Your Category (Casual Dining, UK, 2026)

- Quality-led menus outperforming novelty: guests reward consistency and ingredient quality
- Customisation growing: guided personalisation (hero base + controlled swaps) preferred over unlimited choice
- Experiential elements (limited-time menus, collaborations) increasingly drive visit decisions among under-40s
- 72% of UK diners say they will pay more for fresh and seasonal produce

**Opportunity gap:** Category trend — seasonal: 72% of UK diners say they will pay more for fresh and seasonal produce. No guest mentions detected in current reviews. Worth exploring if this aligns with your offering.

## Public Proof vs Operational Reality

This section compares what customers see (public signals) against what the data reveals about operational depth.

| Aspect | Public Signal | Operational Reality |
|--------|-------------|---------------------|
| Customer Rating | 4.0/5 Google (Good) | Experience 8.8/10 (Strong) — Gap |
| Hygiene Rating | FSA 5/5 (Top mark) | Trust 8.1/10 (Strong) |
| Review Volume | 73 reviews (Low) | Visibility 5.1/10 (Weak) |
| Booking/Ordering | What customers can find | Conversion 6.0/10 (Partial) |

## Category & Peer Validation

### Why This Venue Is In This Cohort

Simla Takeaway is classified as **Restaurant (General)** (confidence: high). Google's raw category is "Indian Restaurant" but the resolved category triangulates across 2 signal sources.

### Category Evidence

| Signal Source | Category Signal | Weight |
|---|---|---|
| Google Place Types | meal_takeaway, indian_restaurant, restaurant | Primary |
| Review Language | "restaurant" (1x) | Supportive |

**Resolved category:** Restaurant (General) (confidence: high)

### Peer Justification

- **Avon Spice** (6.4): Dining with comparable price band; 0.0 miles away
- **Shah's** (6.3): Dining with comparable price band; 4.9 miles away
- **Hussains** (6.2): Takeaway-focused — different service model; 0.2 miles away *(weak comparator)*
- **Kolshi Restaurant** (6.1): Dining with comparable price band; 5.0 miles away
- **Thespians Indian Restaurant** (5.4): Dining with comparable price band; 0.3 miles away

## Competitive Market Intelligence

**Local market:** 6 direct competitors identified.

You are in the top quintile (P100). The competitive risk is complacency — the nearest competitor scores 6.4.

**Where you lead:** Conversion (+2.4), Visibility (+1.6)

## Market Position

**Category:** Indian Restaurant

### Local Competitive Set (6 peers within 5 miles)

**Position:** #1 of 7 (Percentile 100.0)

You are in the top quintile of this peer set. Your overall score of 6.6 exceeds the peer average of 5.9 by 0.7 points. The competitive risk here is complacency — the nearest competitor scores 6.4.

**Dimension gaps vs peers:**

- Conversion: 6.0 (+2.4 above peer avg 3.6)
- Visibility: 5.1 (+1.6 above peer avg 3.5)

**Closest competitors:**

- Avon Spice (6.4) — you lead by 0.2
- Shah's (6.3) — you lead by 0.3
- Hussains (6.2) — you lead by 0.4
- Kolshi Restaurant (6.1) — you lead by 0.5
- Thespians Indian Restaurant (5.4) — you lead by 1.2

### Competitor Strategy

**Common pattern:** Multiple local competitors lead you on **experience**. This is your most consistent competitive gap — addressing it would improve your position against the field, not just one rival.

**Avon Spice** (signal-backed)

- **Wins on:** Trust (+0.8): same FSA rating but likely stronger sub-scores or more recent inspection; Experience (+0.6): Google 4.5/5 vs your 4.0/5
- **Why it matters:** They lead on both Trust and Experience — a combined advantage that is harder to close than a single-dimension gap.
- **Copy / test:** Review inspection documentation and sub-score gaps. Read their recent positive reviews (4.5/5) for operational clues.
- **Defend:** Defend your conversion readiness (+2.5) — you are easier to book/find and your visibility advantage (+1.7).
- *Based on Google rating/reviews, FSA record, 4 dimension scores*

**Shah's** (signal-backed)

- **Wins on:** Experience (see pattern above)
- **Why it matters:** Their experience lead means guests rate their visit higher — this drives reviews, repeats, and word-of-mouth.
- **Copy / test:** Read their recent positive reviews (4.7/5) for operational clues.
- **Defend:** Defend your conversion readiness (+2.5) — you are easier to book/find and your visibility advantage (+1.6).
- *Based on Google rating/reviews, FSA record, 4 dimension scores*

**Cardamom** (signal-backed)

- **Wins on:** Experience (see pattern above)
- **Why it matters:** Their experience lead means guests rate their visit higher — this drives reviews, repeats, and word-of-mouth.
- **Copy / test:** Read their recent positive reviews (4.6/5) for operational clues.
- **Defend:** Defend your visibility advantage (+1.6) and your conversion readiness (+1.0) — you are easier to book/find.
- *Based on Google rating/reviews, FSA record, 4 dimension scores*

### Extended Catchment (15 peers within 15 miles)

**Position:** #1 of 16 (Percentile 100.0)

You are in the top quintile of this peer set. Your overall score of 6.6 exceeds the peer average of 5.6 by 1.0 points. The competitive risk here is complacency — the nearest competitor scores 6.6.

**Dimension gaps vs peers:**

- Conversion: 6.0 (+2.3 above peer avg 3.7)
- Visibility: 5.1 (+1.8 above peer avg 3.2)
- Trust: 8.1 (+1.2 above peer avg 6.9)

**Closest competitors:**

- Cardamom (6.6) — you lead by 0.0
- Avon Spice (6.4) — you lead by 0.2
- Shah's (6.3) — you lead by 0.3
- Hussains (6.2) — you lead by 0.4
- The Spice Room (6.1) — you lead by 0.5

### UK Category Cohort (16 peers in Indian Restaurant)

**Position:** #1 of 17 (Percentile 100.0)

You are in the top quintile of this peer set. Your overall score of 6.6 exceeds the peer average of 5.5 by 1.0 points. The competitive risk here is complacency — the nearest competitor scores 6.6.

**Dimension gaps vs peers:**

- Conversion: 6.0 (+2.3 above peer avg 3.7)
- Visibility: 5.1 (+1.9 above peer avg 3.2)
- Trust: 8.1 (+1.2 above peer avg 7.0)

**Closest competitors:**

- Cardamom (6.6) — you lead by 0.0
- Avon Spice (6.4) — you lead by 0.2
- Shah's (6.3) — you lead by 0.3
- Hussains (6.2) — you lead by 0.4
- The Spice Room (6.1) — you lead by 0.5

## Dimension Scorecard

| Dimension | Score | Δ | Peer Avg | Gap | Read |
|-----------|------:|--:|---------:|----:|------|
| Experience | 8.8 | — | 8.6 | +0.2 | Strong |
| Visibility | 5.1 | — | 3.5 | +1.6 | Clear strength |
| Trust | 8.1 | — | 7.9 | +0.3 | Above peers |
| Conversion | 6.0 | — | 3.6 | +2.4 | Clear strength |
| **Overall** | **6.6** | **—** | | | |

*Prestige (editorial recognition): 0.0/10 (peer avg 0.8) — tracked but not a headline operational lever for most independents.*

## Dimension-by-Dimension Diagnosis

### Experience — 8.8/10

*Customer-facing quality — food, service, ambience as perceived by guests*

**Assessment: Strong.** This is a genuine competitive strength at 8.8.
 In line with peers (avg 8.6).

**Why it matters:** Drives reviews, repeat visits, and word-of-mouth. The most commercially impactful dimension.

**Driven by:** Google rating, TripAdvisor rating, FSA food hygiene sub-score, sentiment analysis

**Management note:** Strong experience but peers are close. Monitor your most recent reviews for early signs of drift — the last 10 reviews matter more than the aggregate.


### Visibility — 5.1/10

*How easily customers can discover you online*

**Assessment: Weak.** At 5.1, this dimension is actively limiting your overall position.
 You lead peers by 1.6 points (peer avg 3.5).

**Why it matters:** No visibility = no new customers. Controls top-of-funnel demand generation.

**Driven by:** Google review count, photo count, GBP completeness, web/social presence, TripAdvisor presence

**Management note:** At 73 reviews, you are likely invisible in 'near me' and category searches. This is fixable within 60 days with a systematic review request programme.


### Trust — 8.1/10

*Food safety compliance and regulatory confidence*

**Assessment: Strong.** This is a genuine competitive strength at 8.1.
 In line with peers (avg 7.9).

**Why it matters:** A hygiene rating below 5 is visible to customers. Low trust creates a hard ceiling on overall score.

**Driven by:** FSA hygiene rating, structural compliance, management confidence, inspection recency

**Management note:** See *Trust — Behind the Headline* below for full decomposition, peer comparison, and action options.


### Conversion — 6.0/10

*Operational readiness to capture demand*

**Assessment: Adequate.** At 6.0, this is functional but not distinctive.
 You lead peers by 2.4 points (peer avg 3.6).

**Why it matters:** Converts interest into visits. Missing hours/menu/ordering options cause drop-off at purchase intent.

**Driven by:** Opening hours, menu online, delivery/takeaway, reservations, price level

**Management note:** Some operational signals present, but gaps remain. Each missing element is a percentage of potential customers who never complete their intent.


### Prestige (Editorial Recognition)

*Score: 0.0/10 | Peer avg: 0.8*

Low prestige is normal for most independents and does not affect footfall, discovery, or day-to-day revenue. Not commercially urgent.

### Trust Dimension — Behind the Headline

**What customers see:** FSA 5/5 — the top hygiene mark. This is a strong public signal and should be protected.

**What the data shows behind it:**

| Subcomponent | Score | Status | Note |
|---|---|---|---|
| FSA Headline Rating | 10.0/10 | ✅ Strong | Top mark — no action needed |
| Inspection Recency | 3.7/10 | ⚠️ Significant drag | Last inspected 2025-01-30 (14 months ago). Score starting to decay. Note it b... |
| Structural Compliance | 8.0/10 | ✅ Adequate | Strong — premises, facilities, equipment all adequate. |
| Confidence in Management | 10.0/10 | ✅ Strong | Strong management systems and training record. |

**Biggest drag:** Inspection Recency. Last inspected 2025-01-30 (14 months ago). Score starting to decay. Note it but don't escalate. 

**What you can do:**

2. **Display your 5/5 prominently** — physical sticker at entrance, add to Google Business Profile, mention in review responses. Costs nothing.
3. **Do nothing** — viable. The 5/5 headline protects you with customers. The gap is competitive, not critical.

**Commercial read:** This is not a revenue problem today. It's a competitive vulnerability — and an easy win if you want it. Estimated score recovery: +1.6 on trust, +0.4 on overall (6.6 → 7.0).

### Business Health Signals

*Companies House data not yet matched for this venue. This will populate once the collection script is run with a valid API key. If you know your Companies House registration number, contact us to link it manually.*

## Watch List

**Monitor local competitor openings and closures** [NEW]

New entrants or closures within 5 miles can shift your position. Stay aware of planning applications and social media.

**Track Google rating trend month-over-month** [NEW]

A sustained 0.1-point drop over 3 months indicates systemic issues before they become obvious.

## What Not to Do This Month

**Don't chase prestige before fixing fundamentals**

Awards don't fix a weak proposition — they amplify a strong one. Get Experience, Trust, and Visibility above 7.0 first.

## Implementation Framework

3 active items: **3 new/recent**.

### Action 1: Lean into food quality — it's what guests are buying
**Status:** New | **Priority:** High
**Target date:** 08 April 2026 | **Cost:** Low (< £200) | **Expected upside:** Sharpen proposition → attract the right guests
**Owner:** management — head chef + FOH manager to review together
**Next milestone:** Pull up the 3 most recent negative reviews. Identify the common theme. Brief the head chef and FOH manager together on the pattern this week
**Success measure:** No food quality complaints in next 10 reviews (verifiable via sentiment analysis)

### Action 2: Accelerate review volume — you're credible but not dominant
**Status:** New | **Priority:** Medium
**Target date:** 08 April 2026 | **Cost:** Low (< £200) | **Expected upside:** Improved Google Maps ranking
**Owner:** front-of-house — whoever currently responds to online reviews
**Next milestone:** Identify the single most actionable sub-task and assign it to a named person with a deadline
**Success measure:** Google review count increased by 10+ (verifiable via grc field)

### Action 3: Address isolated food quality complaints from guests
**Status:** New | **Priority:** Standard
**Target date:** 01 May 2026 | **Cost:** Medium (£200–£1,000) | **Expected upside:** Reduce food quality complaints, protect rating
**Owner:** operations — head chef + FOH manager to review together
**Next milestone:** Pull up the 3 most recent negative reviews. Identify the common theme. Brief the head chef and FOH manager together on the pattern this week
**Success measure:** No food quality complaints in next 10 reviews (verifiable via sentiment analysis)

## Next-Month Monitoring Plan

These are **externally observable leading indicators** — all trackable without internal systems. Changes here signal whether actions are landing.

| Indicator | Current | What Movement Means | Source |
|-----------|---------|---------------------|--------|
| Google star rating | 4.0/5 | Drop below 4.0 = experience drift; rise = guest improvements landing | Google Maps |
| Google review count | 73 | 10+ new reviews/month = healthy momentum | Google Maps |
| Latest 5 Google reviews | Check manually | 2+ negative in latest 5 = early warning of rating decline | Google Maps |
| Nearest competitor rating | Check top 3 | Competitor improvement narrows your lead | Google Maps |

**Review date:** First week of next month. All indicators are publicly observable — no operator login or internal data required.

---

## Data Coverage & Confidence

| Source | Status | Provenance | Available |
|--------|--------|------------|:---------:|
| FSA Hygiene Rating | Rating 5/5 | observed | ✓ |
| Google Business Profile | 4.0★ (73 reviews) | observed | ✓ |
| Google Review Text | 5 reviews analyzed | observed | ✓ |
| TripAdvisor | Not collected | not_assessed | — |
| Companies House | Not checked | not_assessed | — |

**Report confidence: Medium** — 2 independent sources (FSA, Google). Additional platforms would strengthen cross-validation.

**What additional collection would unlock:**

- **TripAdvisor enrichment** → cross-platform validation, convergence scoring


## Evidence Appendix

Raw signal values used to compute this report. Provenance: **observed** = directly from source API; **derived** = computed from observed data; **inferred** = estimated from indirect evidence; **not_assessed** = not yet collected.

| Signal | Value | Source | Provenance |
|--------|-------|--------|------------|
| FSA Hygiene Rating | 5/5 | FSA API via Firebase | observed |
| Food Hygiene Sub-score | 10.0/10 | FSA | observed |
| Structural Compliance | 8.0/10 | FSA | observed |
| Management Confidence | 10.0/10 | FSA | observed |
| Last Inspection | 2025-01-30 | FSA | observed |
| Google Rating | 4.0/5 | Google Places API | observed |
| Google Reviews | 73 | Google Places API | observed |
| Google Photos | 7 | Google Places API | observed |
| GBP Completeness | 8.0/10 | Computed | derived |
| Website | Yes | Google inference | inferred |
| Facebook | No | Google inference | inferred |
| Instagram | No | Google inference | inferred |
| Category | Indian Restaurant | google_types | derived |
| Postcode | CV37 6LE | FSA | observed |
| Companies House | — | Not checked | not_assessed |

### Topic Summary

| Topic | Signal | Mentions |
|-------|--------|--------:|
| Food Quality | strong positive signal | 4 |
| Service & Hospitality | positive signal, limited evidence | 2 |
| Value for Money | positive signal, limited evidence | 1 |

---

### Full Review Text Appendix

*Complete text of each analysed review. Provided for transparency and audit — the summary table above is the primary reference.*

**Review 1** | Unknown | Undated | 4★ | positive | Food Quality
> Takeaway only, no dine in option.

Food is excellent, fresh produce (fresh salad/sides, etc), well spiced and served piping hot.     Extensive menu and above all, everything tasted great.

I was staying in a nearby hotel overnight and brought the food back to my room, so if staying in Stratford and don't want to use the bland hotel menus, then this is worth a visit.

**Review 2** | Unknown | Undated | 5★ | positive | Service & Hospitality
> Honestly the best Indian in Stratford! Never had a bad meal. A lot of places are hit and miss but not this place. Curries nice and thick sauce and the delivery guys are super friendly and always bring up to my flat for me. Lamb tikka makhani is amazing!

**Review 3** | Unknown | Undated | 5★ | positive | Food Quality
> The BEST takeaway in Stratford, absolutely gorgeous food and happy smiling staff. No need to order from anywhere else. Thank you. The Real Ron 👍🙏

**Review 4** | Unknown | Undated | 1★ | negative
> Ordered through just eat and found glass in my food, ordered before and has always been amazing however really not happy about this and when I tried to contacting them they said there was nothing they could do (contacted through just eat)

**Review 5** | Unknown | Undated | 5★ | positive | Food Quality, Service & Hospitality, Value for Money
> We always have great food from Simla!  Definitely our favourite Indian takeaway in Stratford-on-Avon! The drivers are very professional and very friendly! Great value for money too.

## Appendix: How Scores Work

The overall score is a weighted composite of four headline dimensions plus a tracked prestige signal:

| Dimension | Weight | What It Measures |
|---|---|---|
| Experience | ~30% | Food quality, service, ambience — from ratings, reviews, and FSA food hygiene |
| Trust | ~25% | FSA compliance record, inspection recency, management confidence |
| Visibility | ~20% | Online discoverability — review volume, photos, profile completeness |
| Conversion | ~15% | Operational readiness — hours, menu, reservations, delivery/takeaway |
| Prestige | ~10% | Awards, editorial recognition (tracked, not a headline lever) |

**How scores move:** Scores change when underlying data changes (new reviews, new inspection, profile updates). The biggest levers are Experience (~30%) and Trust (~25%). Some factors decay over time (e.g., inspection recency) even without new data. Peer averages shift as competitors improve or decline.

**What scores don't capture:** Internal operational data, financial performance, staff quality beyond what's visible in reviews. This is an outside-in view only.

*Methodology: DayDine Premium v3.3. Full methodology available on request.*


*Report generated by DayDine v3.3 — 2026-04*